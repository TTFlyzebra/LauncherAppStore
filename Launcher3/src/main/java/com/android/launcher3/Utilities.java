...
public final class Utilities {
    ...
    public static Bitmap createMin3rdIconBitmap(Drawable icon, Context context) {
        synchronized (sCanvas) { // we share the statics :-(
            if (sIconWidth == -1) {
                initStatics(context);
            }
            ...
            sOldBounds.set(icon.getBounds());
            icon.setBounds(Math.min(left + margin, margin), Math.min(top + margin, margin),
                    Math.max(sIconWidth - margin, left + width - margin), Math.max(sIconHeight - margin, top + height - margin));
            icon.draw(canvas);
            icon.setBounds(sOldBounds);
            canvas.setBitmap(null);
            return bitmap;
